  ...
  (define (draw-in dc x-mid y-mid total-depth using-color remaining-depth)
    (if (< remaining-depth 0) (void)
        (let ()
            ...
            ;; Draw the arms
            (send dc set-pen (make-pen #:color using-color #:width (* 10 general-decay-weight 1.2)))
            (send dc draw-line x-mid y-mid hour-arm-end-x hour-arm-end-y)
            (send dc set-pen (make-pen #:color using-color #:width (* 10 general-decay-weight 0.8)))
            (send dc draw-line x-mid y-mid minute-arm-end-x minute-arm-end-y)
            (send dc set-pen (make-pen #:color using-color #:width (* 10 general-decay-weight 0.6)))
